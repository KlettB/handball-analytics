...
export default function Players() {
  ...
  const colColors = {
    ...
    seven_meter_goals: 'text-green-300',
    seven_meter_missed: 'text-red-400',
    two_minute_penalties: 'text-yellow-400',
    warnings: 'text-yellow-300',
    disqualifications: 'text-red-500',
    games_played: 'text-gray-300',
  };
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-700 text-xs text-gray-400">
              <th
                className={`text-left px-4 py-3 cursor-pointer hover:text-white ${sortKey === 'name' ? 'text-white' : ''}`}
              ...
              {cols.map((c) => (
                <th
                  key={c.key}
                  className={`px-2 py-3 text-center w-10 cursor-pointer hover:text-white ${sortKey === c.key ? 'text-white' : ''}`}
                  title={c.title}
            ...
            {sorted.map((p) => (
              ...
              >
                ...
                {cols.map((c) => (
                  <td
                    key={c.key}
                    className={`px-2 py-2.5 text-center ${p[c.key] > 0 ? colColors[c.key] : 'text-gray-600'}`}
                  >
                    {p[c.key] > 0 ? p[c.key] : '–'}
                  </td>
                ))}
